...
#
# The image is pulled rather than built: the upstream postgres images are
# multi-architecture, so the host's own architecture runs natively and there is
# no emulation on this path and no Dockerfile to maintain.
#
# The major version is not free to choose. The entry tables declare their
# payload column `bytea STORAGE PLAIN`, which is what keeps a TOAST relation
# from ever being created, and per-column STORAGE inside CREATE TABLE is a
# syntax error before PostgreSQL 16. The check below fails loudly rather than
# letting the suite discover it as a parse error at CreateLogs time.
set -eu -o pipefail
...
# Poll until `probe` prints something containing `marker`, giving up after two
# minutes. The probe's output is captured rather than piped into `grep -q`: grep
# exits on the first match and SIGPIPEs the writer, which under `set -o pipefail`
# would mark the check failed even when the marker is there.
wait_for() {
...
# tmpfs: this keeps the disk out of the way of a suite that exists to exercise
# the design, not the storage.
...
# The probe also proves the point of the clause: no toast relation was created.
